...
@api_view(['GET'])
# when user signed in authnticate token
@authentication_classes([authentication.TokenAuthentication])
@permission_classes([permissions.IsAuthenticated])
def generate_pdf(request, invoice_id):
    # get_object_or_404 will return error if invoice does not exist
    invoice = get_object_or_404(Invoice, pk=invoice_id, created_by=request.user)
    team = Team.objects.filter(created_by=request.user).first()

    template_name = 'pdf.html'

# if is_credit_for is true we generate the creditnote pdf instead
